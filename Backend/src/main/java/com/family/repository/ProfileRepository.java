...
public interface ProfileRepository extends JpaRepository<Profile, UUID>, JpaSpecificationExecutor<Profile> {
    ...
    List<ProfileLookupResponse> findProjectedByDeletedFalse();
    
    @Query("SELECT p.id as id, p.fullName as fullName, p.gender as gender, p.generation as generation, p.parentId as parentId " +
           "FROM Profile p " +
           "WHERE p.deleted = false AND (p.id = :currentId OR p.id = (SELECT p2.spouseId FROM Profile p2 WHERE p2.id = :currentId))")
    List<ProfileLookupResponse> findLookupByMemberAndSpouse(@Param("currentId") UUID currentId);
