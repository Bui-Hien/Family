...
import com.family.dto.response.ProfileResponse;
...
public class ProfileController {
    ...
    public ResponseEntity<ApiResponse<ProfileResponse>> getCurrentProfile() {
        com.family.entity.User currentUser = SecurityUtils.getCurrentUser()
                .orElseThrow(() -> new com.family.common.exception.UnauthorizedException("User not authenticated"));
        if (currentUser.getProfileId() == null) {
            return ResponseEntity.ok(ApiResponse.success(null));
        }
        Profile profile = profileService.getById(currentUser.getProfileId());
    ...
    public ResponseEntity<ApiResponse<List<ProfileLookupResponse>>> getLookup(@RequestParam(required = false) UUID currentId) {
